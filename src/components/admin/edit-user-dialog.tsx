...
export function EditUserDialog({ isOpen, setIsOpen, user, onUserUpdated }: EditUserDialogProps) {
  ...
  React.useEffect(() => {
    const fetchAndSetUserData = async () => {
        if (user && isOpen) {
            try {
                const endpoint = user.role === 'etudiant' ? `/admin/etudiant/${user.id}` : `/admin/professeur/${user.id}`;
                const details = await apiFetch(endpoint);
                form.reset({
                    ...user,
                    ...details,
                    mot_de_passe: '', // Ne jamais pré-remplir le mot de passe
                } as FormValues);
            } catch (error: any) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    if (values.mot_de_passe) {
      user_update.mot_de_passe = values.mot_de_passe;
    }
                ...
                <ScrollArea className="h-[60vh] pr-6">
                    <div className="space-y-4 py-4">
                        ...
                        {role === 'etudiant' && (
                            <>
                                <hr className="my-4"/>
                                <FormField control={form.control} name="matricule" render={({ field }) => ( <FormItem><FormLabel>Matricule</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormMessage> )} />
                                <div className="grid grid-cols-2 gap-4">
                                ...
                                <FormField control={form.control} name="niveau" render={({ field }) => ( <FormItem><FormLabel>Niveau</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue /></SelectTrigger></FormControl><SelectContent>{['L1', 'L2', 'L3', 'M1', 'M2'].map(v=><SelectItem key={v} value={v}>{v}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem> )} />
                        ...
                        <FormField control={form.control} name="photo_url" render={({ field }) => ( <FormItem><FormLabel>URL de la photo (Optionnel)</FormLabel><FormControl><Input {...field} /></FormControl><FormMessage /></FormMessage> )} />
                    </div>
                </ScrollArea>
